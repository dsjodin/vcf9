...
Function Get-OrCreate-WLDNetworkPool {
    ...
    if ($existingPool) {
        My-Logger "Found existing WLD Network Pool '$PoolName' (ID: $($existingPool.id))"
        return $existingPool.id
    }

    My-Logger "WLD Network Pool '$PoolName' not found, creating ..."
    ...
    My-Logger "Creating WLD Network Pool with spec:`n$body" "yellow"
    ...
    My-Logger "Created WLD Network Pool '$PoolName' (ID: $($newPool.id))"
    return $newPool.id
}
...
if($generateWldHostCommissionJson -eq 1) {
    ...
    try {
        ...
        My-Logger "Using WLD Network Pool '$VCFWorkloadDomainPoolName' (ID: $wldPoolId)"
    } catch {
...
if($generateWLDDeploymentFile -eq 1) {
    ...
    # --------------------------------------------------------------------
    # Derive WLD ESXi NSX TEP network details from CIDR  <-- ADD HERE
    # --------------------------------------------------------------------
