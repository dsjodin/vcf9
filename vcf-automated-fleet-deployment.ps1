...
if($deployVCFInstaller -eq 1) {
    ...
    $ovfconfig.vami.SDDC_Manager.gateway.value = $VMMGMTGateway
...
if($deployNestedESXiVMsForMgmt -eq 1) {
    $NestedESXiHostnameToIPsForManagementDomain.GetEnumerator() | Sort-Object -Property Value | Foreach-Object {
		...
		My-Logger "Updating Virtual Hardware storage for Nested ESXi VMs (Boot Disk=${NestedESXiMGMTBootDisk}GB vSAN Capacity=${NestedESXiMGMTCapacityvDisk}GB) ..."
		...
		# Remove cache disk (Hard disk 2) since we're using vSAN ESA
		My-Logger "Removing cache disk (Hard disk 2) for vSAN ESA configuration ..."
		Get-HardDisk -Server $viConnection -VM $vm -Name "Hard disk 2" | Remove-HardDisk -Confirm:$false | Out-File -Append -LiteralPath $verboseLogFile

		# Resize capacity disk (which becomes Hard disk 2 after removal)
		Get-HardDisk -Server $viConnection -VM $vm -Name "Hard disk 2" | Set-HardDisk -CapacityGB $NestedESXiMGMTCapacityvDisk -Confirm:$false | Out-File -Append -LiteralPath $verboseLogFile
...
if($deployNestedESXiVMsForWLD -eq 1) {
    $NestedESXiHostnameToIPsForWorkloadDomain.GetEnumerator() | Sort-Object -Property Value | Foreach-Object {
		...
		My-Logger "Updating Virtual Hardware storage for Nested ESXi VMs (Boot Disk=${NestedESXiWLDBootDisk}GB vSAN Capacity=${NestedESXiWLDCapacityvDisk}GB) ..."
		...
		# Resize capacity disk (which becomes Hard disk 2 after removal)
		Get-HardDisk -Server $viConnection -VM $vm -Name "Hard disk 2" | Set-HardDisk -CapacityGB $NestedESXiWLDCapacityvDisk -Confirm:$false | Out-File -Append -LiteralPath $verboseLogFile
